...
- 量化策略借鉴 Longbridge OpenAPI / Terminal、FinRL-Trading、Lean、Qlib、vn.py、PyPortfolioOpt 的是券商网关、信号组织、研究合同和边界设计，不是把这些框架的执行层搬进来。
...
对于自选池量化自动执行，这个服务只接受来自上游的受控下单请求，并继续应用账户、权限、现金、重复委托和仓位边界。上游美股开盘自动交易在提交前必须刷新券商实时价并通过日内订单/金额护栏；若任一条件不满足，订单不进入 broker 写动作。
