...
class RedisClient:
    ...
    def _store_fallback_value(self, key: str, value: Any, expire: int | None = None) -> None:
        ...
        try:
            ...
            if config.get("cluster_enabled"):
                ...
                self._client = RedisCluster(
                    startup_nodes=startup_nodes, password=config["password"] or None, decode_responses=True
                )
                self._pool = None
    ...
    # String操作
    def set(self, key: str, value: str | int | float, expire: int | None = None) -> bool:
        """设置值"""
        try:
    ...
    def set_hot(self, key: str, value: str | int | float | dict | list, expire: int | None = 30) -> bool:
        """写入热数据缓存。"""
        return self.set(self._scoped_key("hot", key), value, expire=expire)

    def get_hot_json(self, key: str) -> Any | None:
        """读取热数据缓存。"""
        return self.get_json(self._scoped_key("hot", key))

    def set_cold(self, key: str, value: str | int | float | dict | list, expire: int | None = 3600) -> bool:
        """写入冷数据缓存。"""
        return self.set(self._scoped_key("cold", key), value, expire=expire)

    def get_cold_json(self, key: str) -> Any | None:
        """读取冷数据缓存。"""
...
def redis_lock(lock_key: str, expire: int = 30):
    ...
    try:
        ...
        while not lock_acquired:
            lock_acquired = redis_client.client.set(lock_key, identifier, nx=True, ex=expire)
