            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    if attempt < max_retries:
                        logger.warning(
                            f"{func.__name__} 第{attempt + 1}次尝试失败: {e}, " f"{current_delay}秒后重试..."
                        )
...
def handle_api_error(
    default_return: Any = None, log_level: str = "ERROR", error_message: str | None = None
) -> Callable:
    """
    API错误处理装饰器
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs) -> Any:
            try:
                ...
                log_error(logger, e, {"context": "api_error", "function": func.__name__, "status_code": e.status_code})
                ...
                log_error(logger, e, {"context": "network_error", "function": func.__name__})
                ...
                log_error(logger, e, {"context": "unexpected_error", "function": func.__name__})
...
def safe_execute(
    func: Callable, default_return: Any = None, error_handler: Callable | None = None, *args, **kwargs
) -> Any:
    """
    安全执行函数
    ...
    def __call__(self, func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs) -> Any:
            if not self.can_execute():
                raise NetworkError("熔断器打开，服务暂时不可用")
