...
class MarketUniverseSync:
    ...
    def sync_markets(cls, markets: Sequence[str] | None = None, user_id: int = 1) -> dict[str, object]:
        ...
        result = {"markets": {}, "total_saved": 0, "started_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S")}
        ...
        result["warnings"] = [
            warning for market_result in result["markets"].values() for warning in market_result.get("warnings", [])
        ]
    ...
    def sync_market(cls, market: str, user_id: int = 1) -> dict[str, object]:
        ...
        stock_saved = cls._upsert_new_rows(
            cls.STOCK_MARKET_TABLES[normalized_market], stock_rows, source=stock_df.attrs.get("source", "unknown")
        )
        ...
        etf_saved = cls._upsert_new_rows(
            cls.ETF_MARKET_TABLES[normalized_market], etf_rows, source=etf_df.attrs.get("source", "unknown")
        )
        warnings = []
        ...
        return {
            ...
            "fetched": {"stocks": int(len(stock_df.index)), "etfs": int(len(etf_df.index))},
            "normalized": {"stocks": len(stock_rows), "etfs": len(etf_rows)},
...
""".strip()
    ...
    def _skshare_base_urls(cls) -> list[str]:
        configured = os.getenv("SKSHARE_BASE_URL") or os.getenv("REF_SKSHARE_BASE_URL") or cls.SKSHARE_DEFAULT_BASE_URL
        candidates = []
    ...
    def _market_universe_skshare_timeout(cls) -> int:
        raw = os.getenv("MARKET_UNIVERSE_SKSHARE_TIMEOUT") or os.getenv("REF_MARKET_UNIVERSE_SKSHARE_TIMEOUT")
        if raw:
            try:
    ...
    def _normalize_dataframe(
        cls, market: str, dataframe: pd.DataFrame, user_id: int = 1, asset_type: str = "stock"
    ...
    def _upsert_rows(cls, table_config: dict[str, str], rows: list[dict[str, object]]) -> int:
        ...
        update_clause = ", ".join([f"{column} = VALUES({column})" for column in columns if column != "symbol"])
        sql = f"""
