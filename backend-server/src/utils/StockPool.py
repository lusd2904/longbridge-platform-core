...
class StockPool:
    ...
    def get_all_stocks_from_db():
        ...
        try:
            # 获取所有以.US结尾或market='US'的股票
            stocks = DbUtil.query_all("SELECT symbol, company_name, market FROM large_cap_stocks WHERE symbol LIKE '%.US' OR market = 'US' ORDER BY symbol")
            for stock in stocks:
                all_stocks.append({
                    'symbol': stock[0],
                    'name': stock[1],
                    'market': stock[2] if stock[2] else 'US'
                })
        except Exception as e:
        ...
        try:
            ...
            try:
                # 获取所有以.SZ或.SH结尾的股票
                stocks = DbUtil.query_all("SELECT symbol, company_name, market FROM large_cap_stocks WHERE symbol LIKE '%.SZ' OR symbol LIKE '%.SH' OR market = 'CN' ORDER BY symbol")
                for stock in stocks:
                    all_stocks.append({
                        'symbol': stock[0],
                        'name': stock[1],
                        'market': stock[2] if stock[2] else 'CN'
                    })
