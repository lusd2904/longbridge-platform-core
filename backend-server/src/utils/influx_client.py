...
class InfluxDBManager:
    ...
    def write_quote(
        ...
        """
        try:
            point = Point("stock_quote").tag("symbol", symbol).field("price", float(price)).field("volume", int(volume))
    ...
    def query_quotes(self, symbol: str, start: datetime, end: datetime = None, interval: str = "1m") -> list[dict]:
        """
        查询行情数据
        :param symbol: 股票代码
        :param start: 开始时间
    ...
    def write_indicator(self, symbol: str, indicator_name: str, value: float, timestamp: datetime = None):
        """
        写入技术指标
        """
        try:
            point = (
                Point("technical_indicator")
                .tag("symbol", symbol)
    ...
    def query_indicators(self, symbol: str, indicator: str, start: datetime, end: datetime = None) -> list[dict]:
        """查询技术指标"""
        try:
            end = end or datetime.utcnow()
    ...
    def write_trade(
        self, order_id: str, symbol: str, side: str, quantity: float, price: float, timestamp: datetime = None
    ):
        """
        写入交易记录
        """
