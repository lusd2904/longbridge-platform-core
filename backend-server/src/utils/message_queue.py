...
class MessageQueue:
    ...
    def _process_task(self, task_data: str):
        ...
        try:
            ...
            # 保存结果（保留24小时）
            redis_client.set(
                f"{self.result_key_prefix}{task.task_id}",
                task.to_dict(),
                expire=86400
            )
...
# 便捷函数
def submit_ai_analysis_task(symbol: str, data: Dict[str, Any], 
...
def submit_trade_task(symbol: str, action: str, quantity: float,
