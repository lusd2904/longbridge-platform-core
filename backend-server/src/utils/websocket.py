...
class WebSocketManager:
    ...
    @staticmethod
    def _normalize_symbols(symbols: Optional[list]) -> list:
        normalized = []
    ...
    def unregister(self, ws, symbols: list = None, user_id: str = None):
        ...
        with self.lock:
            ...
            normalized_user_id = (
                str(user_id) if user_id not in (None, '') else subscription.get("user_id")
            )
    ...
    def get_user_symbol_map(self) -> Dict[Optional[str], Set[str]]:
        """按用户聚合当前订阅的股票代码。"""
        aggregated: Dict[Optional[str], Set[str]] = defaultdict(set)
        with self.lock:
    ...
    def get_stats(self) -> dict:
        ...
        with self.lock:
            return {
                ...
                "connection_counts": {
                    symbol: len(conns)
                    for symbol, conns in self.connections.items()
                }
            }
...
class QuotePusher:
    ...
    def __init__(self, ws_manager: WebSocketManager, interval_seconds: Optional[int] = None):
    ...
    def _refresh_runtime_config(self) -> bool:
        from core.platform.SystemTaskService import SystemTaskService
        self.interval_seconds = max(1, SystemTaskService.get_interval("websocket_quote_stream", settings.WEBSOCKET_QUOTE_INTERVAL))
    ...
    @staticmethod
    def _normalize_quote(symbol: str, quote) -> Optional[dict]:
        if quote is None:
            return None
