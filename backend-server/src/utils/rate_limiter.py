...
class RateLimiter:
    ...
    def __init__(self, key_prefix: str = "rate_limit", strategy: RateLimitStrategy = RateLimitStrategy.SLIDING_WINDOW):
    ...
    def _local_sliding_window_check(self, key: str, limit: int, window: int) -> tuple[bool, dict]:
        now = time.time()
    ...
    def is_allowed(self, key: str, limit: int = 100, window: int = 60) -> tuple[bool, dict[str, Any]]:
        """
        检查是否允许请求
    ...
    def _fixed_window_check(self, key: str, limit: int, window: int) -> tuple[bool, dict]:
        """固定窗口限流"""
        try:
    ...
    def _sliding_window_check(self, key: str, limit: int, window: int) -> tuple[bool, dict]:
        """滑动窗口限流（简化版）"""
        try:
            now = time.time()
            window_start = now - window
    ...
    def _token_bucket_check(self, key: str, limit: int, window: int) -> tuple[bool, dict]:
        """令牌桶限流"""
        try:
    ...
    def decorator(f: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            if not allowed:
                ...
                payload = {"error": "请求过于频繁，请稍后再试", "retry_after": info.get("reset_time", 60)}
...
def circuit_breaker_protect(name: str, failure_threshold: int = 5, recovery_timeout: int = 60):
    """
    熔断保护装饰器
    ...
    def decorator(f: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            if not breaker.can_execute():
                ...
                return jsonify({"error": "服务暂时不可用，请稍后再试", "circuit_breaker": breaker.get_state()}), 503
