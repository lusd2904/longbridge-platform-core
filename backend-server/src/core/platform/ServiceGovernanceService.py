    ...
    def get_snapshot(cls) -> Dict[str, object]:
        ...
        return {
            ...
            "services": {
                ...
                "websocket": {
                    **quote_pusher.get_runtime_status(),
                    "connections": ws_manager.get_stats()
                }
            },
    ...
    def _list_circuits() -> List[Dict[str, object]]:
        ...
        for name, breaker in sorted(circuit_breakers.items(), key=lambda item: item[0]):
            ...
            items.append({
                ...
                "lastFailureAt": datetime.fromtimestamp(last_failure).strftime("%Y-%m-%d %H:%M:%S") if last_failure else None
            })
        return items
