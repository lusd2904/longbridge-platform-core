...
class AiConsultant:
    ...
    def get_final_decision_with_details(symbol, algo_side, data):
        ...
        # 抛弃三层调用，直接一次性发送给大模型 (Unified Scan)
        combined_text = AIAnalyst.get_decision(None, combined_prompt, task="scan_final", user_id=user_id)

        pulse_text = "N/A"
        risk_text = "N/A"
        decision_text = combined_text

        MonitorLink.log(f"   [Unified Scan决策]: {decision_text[:240]}")
        verdict = AiConsultant._extract_verdict(decision_text, fallback=algo_side)

        # 为了兼容前端数据结构，构造虚假的pulse和risk
        pulse_analysis = {"trend": "量化直出", "details": "已合并至终审"}
        risk_analysis = {"risk": "量化直出", "details": "已合并至终审"}
        decision_analysis = AiConsultant._build_decision_analysis(decision_text, data, market_context, verdict)

        reason = f"市场环境{market_context.get('summary', '中性')}，" f"量化引擎与AI联合终审给出{verdict}。"

        try:
            from utils.DbUtil import DbUtil

            DbUtil.add_web_log(f"[AI决策] {symbol} | Unified Scan:{decision_text[:200]}... | 状态:{verdict}")
    ...
    def _build_combined_analysis_prompt(stock_name: str, symbol: str, algo_side: str, indicators_desc: str) -> str:
        return f"""你是顶级量化分析师。
请结合量化特征数据，一次性输出严格格式的个股研判结论，不要多余解释。
...
"""
    ...
    @staticmethod
    def _extract_combined_sections(text: str) -> dict[str, str] | None:
...
"""
        ...
        if AiConsultant._is_unusable_response(
            pulse_text, required_fields=["趋势判断", "指标共振", "大盘联动", "机会窗口", "一句结论", "建议标签"]
        ):
...
"""
    ...
    def _fallback_risk_text(symbol: str, data: dict, market_context: dict) -> str:
        ...
        signal = (
            "HOLD"
            if market_context.get("regime") == "balanced"
            else "BUY"
    ...
    def _fallback_decision_text(
        symbol: str, algo_side: str, data: dict, market_context: dict, pulse_text: str, risk_text: str
    ) -> str:
        ...
        verdict = (
            "BUY"
            if algo_side == "BUY" and market_context.get("regime") != "risk_off"
            else "SELL"
            if algo_side == "SELL"
            else "HOLD"
        )
...
"""
    ...
    def _build_pulse_analysis(text: str, algo_side: str, data: dict, final_verdict: str) -> dict:
        ...
        window = AiConsultant._extract_field(text, [r"机会窗口\s*[:：]\s*(.+?)(?:\n|$)"], default="等待进一步确认")
        summary = AiConsultant._extract_field(text, [r"一句结论\s*[:：]\s*(.+?)(?:\n|$)"], default=text.strip())
    ...
    def _build_risk_analysis(text: str, data: dict, final_verdict: str) -> dict:
        sentiment = AiConsultant._extract_field(text, [r"情绪温度\s*[:：]\s*(.+?)(?:\n|$)"], default="中性")
        flow = AiConsultant._extract_field(
            text,
            [r"资金流与波动\s*[:：]\s*(.+?)(?:\n|$)"],
            default=f"ATR {float(data.get('atr', 0) or 0):.2f} / ROC {float(data.get('roc', 0) or 0):.2f}%",
        )
        risk = AiConsultant._extract_field(text, [r"主要风险\s*[:：]\s*(.+?)(?:\n|$)"], default="需结合仓位谨慎处理")
        position_advice = AiConsultant._extract_field(text, [r"仓位建议\s*[:：]\s*(.+?)(?:\n|$)"], default="轻仓试探")
        market_env = AiConsultant._extract_field(text, [r"市场环境\s*[:：]\s*(.+?)(?:\n|$)"], default="市场波动中")
        summary = AiConsultant._extract_field(text, [r"一句结论\s*[:：]\s*(.+?)(?:\n|$)"], default=text.strip())
        signal = AiConsultant._extract_verdict(text, fallback=final_verdict)
    ...
    def _build_decision_analysis(text: str, data: dict, market_context: dict, verdict: str) -> dict:
        trend = AiConsultant._extract_field(text, [r"趋势判断\s*[:：]\s*(.+?)(?:\n|$)"], default="震荡整理")
        indicators = AiConsultant._extract_field(
            text,
            [r"关键指标\s*[:：]\s*(.+?)(?:\n|$)"],
            default=f"RSI {float(data.get('rsi', 0) or 0):.1f}, MACD {float(data.get('macd', 0) or 0):.3f}",
        ...
        target = AiConsultant._extract_field(
            text, [r"目标价位\s*[:：]\s*(\$?[\d,.]+)"], default=f"${float(data.get('resistance', 0) or 0):.2f}"
        )
        stop_loss = AiConsultant._extract_field(
            text, [r"止损价位\s*[:：]\s*(\$?[\d,.]+)"], default=f"${float(data.get('support', 0) or 0):.2f}"
        )
        detail_reason = AiConsultant._extract_field(text, [r"详细理由\s*[:：]\s*(.+?)(?:\n*$|$)"], default=text.strip())
