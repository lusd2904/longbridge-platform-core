...
class MarketInsightService:
    ...
    def refresh_all_markets(cls, user_id: int = 1, source: str = "scheduler") -> dict[str, object]:
        ...
        cls._prune_history()
        return {"generated_at": generated_at.strftime("%Y-%m-%d %H:%M:%S"), "markets": payload}

    @classmethod
    def get_latest_snapshots(cls, user_id: int = 1) -> list[dict[str, object]]:
        cls.ensure_schema()
        latest_generated_at_row = (
            DbUtil.fetch_one(
                """
            SELECT generated_at
    ...
    def list_snapshot_points(cls, market: str = "", limit: int = 24) -> list[dict[str, object]]:
        ...
        return [
            {
                "generatedAt": row.get("generated_at").strftime("%Y-%m-%d %H:%M:%S")
    ...
    def build_market_insight(
        cls, market: str, user_id: int = 1, generated_at: datetime | None = None
    ) -> dict[str, object]:
    ...
    def _calculate_market_score(cls, benchmarks: list[dict[str, object]]) -> float:
        ...
        for item in benchmarks:
            weight = next(
                (
                    config["weight"]
                    for config in cls.BENCHMARKS.get(cls._market_from_symbol(item["symbol"]), [])
    ...
    def _compose_headline(cls, market: str, status: dict[str, str], regime: str) -> str:
        regime_text = {"risk_on": "风险偏好回升", "risk_off": "避险情绪升温", "balanced": "市场情绪平衡"}.get(
            regime, "市场情绪平衡"
        )
    ...
    def _compose_summary(
        cls, market: str, status: dict[str, str], regime: str, benchmarks: list[dict[str, object]], market_score: float
    ) -> str:
    ...
    def _get_market_status(cls, market: str) -> dict[str, str]:
        if market == "US":
            ...
            current_minutes = now.hour * 60 + now.minute
            is_open = weekday and (
                (9 * 60 + 30) <= current_minutes < (11 * 60 + 30) or (13 * 60) <= current_minutes < (15 * 60)
            )
        else:
            ...
            current_minutes = now.hour * 60 + now.minute
            is_open = weekday and (
                (9 * 60 + 30) <= current_minutes < (12 * 60) or (13 * 60) <= current_minutes < (16 * 60)
            )

        return {"status": "open" if is_open else "closed", "status_text": "交易中" if is_open else "已休市"}
