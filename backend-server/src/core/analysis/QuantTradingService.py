...
class QuantTradingService:
    ...
    def get_status(cls, user_id: int = 1) -> dict[str, Any]:
        ...
        default_account = next(
            (account for account in accounts if account.get("is_default")), accounts[0] if accounts else None
        )
    ...
    @classmethod
    def run_cycle(
        cls, user_id: int = 1, account_id: int | None = None, source: str = "manual", execute: bool | None = None
    ) -> dict[str, Any]:
    ...
    def run_us_open_watchlist_ai_trade(
        ...
        *,
        ...
        try:
            ...
            targets = cls._load_watchlist_scan_targets(user_id=user_id, session_filter="all")
            targets = [item for item in targets if cls._target_market(item) == policy_settings["market"]][:200]
            evaluations: list[dict[str, Any]] = []
            skipped: list[dict[str, Any]] = []
            ...
            for target in targets:
                if isinstance(target, dict):
                    ...
                    raw_symbol = target
                    target = {"symbol": raw_symbol}
                symbol = cls._normalize_watchlist_symbol(
                    raw_symbol, market=(target or {}).get("market") or policy_settings["market"]
                )
                if not symbol or symbol in seen_symbols:
                    continue
                seen_symbols.add(symbol)
            ...
            if not opportunities:
                ...
                message = (
                    ...
                    if reason == "realtime-price-missing"
                    else "自选股池本轮没有达到买入或卖出条件的标的"
                )
                result = cls._build_us_open_trade_result(
                    cycle_id=cycle_id,
                    source=source,
    ...
    def list_watchlist_strategy_history(cls, user_id: int = 1, limit: int = 20) -> dict[str, Any]:
        ...
        cycle_ids = [str(row.get("cycle_id") or "").strip() for row in rows if str(row.get("cycle_id") or "").strip()]
    ...
    def execute_watchlist_opportunities(
        ...
        *,
        ...
        max_position_ratio: float = 0.08,
        min_confidence: int = 72,
        target_portfolio_ratio: float | None = None,
        allow_sells: bool = False,
        require_paper: bool = False,
        broker: Any = None,
        max_daily_submitted_orders: int | None = None,
        max_daily_notional_ratio: float | None = None,
    ) -> dict[str, Any]:
        cls.ensure_schema()
        access = PlatformAccessService.get_user_capabilities(user_id)
        ...
        safe_target_ratio = (
            ...
            if target_portfolio_ratio is not None
            else None
        )
        safe_daily_order_limit = (
            max(0, int(cls._safe_float(max_daily_submitted_orders, 0))) if max_daily_submitted_orders is not None else 0
        )
        safe_daily_notional_ratio = (
            max(0.0, min(cls._safe_float(max_daily_notional_ratio, 0.0), 1.0))
            if max_daily_notional_ratio is not None
            else 0.0
        )
        ...
        current_market_value = cls._read_number(account_info, "market_value") or sum(
            cls._read_number(position, "market_value") for position in positions
        )
        per_symbol_cap = (
            total_equity * safe_position_ratio if total_equity > 0 and safe_position_ratio > 0 else safe_max_amount
        )
        portfolio_budget = (
            total_equity * safe_target_ratio if safe_target_ratio is not None and total_equity > 0 else None
        )
        ...
        for item in safe_opportunities:
            ...
            if normalized_symbol not in allowed_symbols:
                skipped.append(
                    {"symbol": normalized_symbol or raw_symbol, "reason": "不在当前用户自选股池，禁止自动下单"}
                )
                continue
            ...
            price = float(
                item.get("price") or item.get("lastPrice") or item.get("latestClose") or item.get("closePrice") or 0
            )
            holding = holdings.get(symbol)
                ...
                decisions.append(
                    {
                        ...
                        if isinstance(item.get("scoreBreakdown"), dict)
                        else {},
            ...
            if remaining_portfolio_buy_budget is not None:
                ...
                slot_budget = (
                    remaining_portfolio_buy_budget / remaining_slots if remaining_portfolio_buy_budget > 0 else 0.0
                )
                one_share_budget = (
                    price if remaining_portfolio_buy_budget >= price and available_cash >= price else slot_budget
                )
            ...
            budget_meta = {
                ...
                "remainingPortfolioBuyBudgetBefore": round(remaining_portfolio_buy_budget, 4)
                if remaining_portfolio_buy_budget is not None
                else None,
        ...
        for decision in decisions:
            ...
            guardrail_reason = cls._daily_guardrail_skip_reason(
                daily_guardrail, decision_notional, decision.get("side")
            )
            if guardrail_reason:
            ...
            standard_status = cls._standard_order_status(
                execution.get("standardStatus") or execution.get("status", "failed")
            )
            if standard_status in {"submitted", "accepted", "partially_filled", "filled"}:
                ...
                if str(decision.get("side") or "").strip().upper() == "BUY":
                    daily_guardrail["submittedNotional"] = (
                        float(daily_guardrail.get("submittedNotional") or 0.0) + decision_notional
                    )
        ...
        return {
            ...
            "positionControl": {
                ...
                "budgetRule": (
                    "min(availableCash, perSymbolCap, remainingPortfolioBudget / remainingSlots; US min 1 share)"
                    if safe_target_ratio is not None
                    else "min(maxAmount, availableCash, totalEquity * maxPositionRatio)"
                ),
            },
        }
    ...
    @classmethod
    def _load_us_open_ai_trade_settings(cls, overrides: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        return {
            ...
            "maxDailySubmittedOrders": max(
                0, min(int(cls._safe_float(raw_settings.get("maxDailySubmittedOrders"), 10)), 200)
            ),
            "maxDailyNotionalRatio": max(
                0.0, min(cls._safe_float(raw_settings.get("maxDailyNotionalRatio"), 0.70), 1.0)
            ),
        }
        ...
        *,
        ...
        try:
            ...
            if isinstance(raw_quotes, dict):
                quotes = {
                    HistoricalMarketDataService.normalize_symbol(key): value for key, value in raw_quotes.items() if key
                }
        except Exception as exc:
    ...
    def _load_auto_trade_daily_guardrail(
        ...
        *,
        ...
        max_notional = (
            float(total_equity or 0) * max_notional_ratio if max_notional_ratio > 0 and total_equity > 0 else 0.0
        )
        return {
    ...
    @staticmethod
    def _is_us_regular_session_now(now: datetime | None = None) -> bool:
    ...
    def _build_watchlist_quant_metrics(
        ...
        *,
        ...
        volume_trend20 = (
            round(((avg_volume5 - avg_volume20) / avg_volume20) * 100, 2) if avg_volume5 and avg_volume20 else 0.0
        )
        distance_high20 = (
            round(((latest_close - high20_previous) / high20_previous) * 100, 2)
            if latest_close and high20_previous
            else 0.0
        )
        distance_high60 = (
            round(((latest_close - high60_previous) / high60_previous) * 100, 2)
            if latest_close and high60_previous
            else 0.0
        )
        ...
        boll_percent_b = (
            round(((latest_close - boll_lower) / (boll_upper - boll_lower)) * 100, 2)
            if boll_upper != boll_lower
            else 50.0
        )
        true_range = (
            max(latest_high - latest_low, abs(latest_high - previous_close), abs(latest_low - previous_close))
            if previous_close
            else latest_high - latest_low
        )
        ...
        upper_shadow = (
            round((latest_high - max(latest_open, latest_close)) / latest_close * 100, 2) if latest_close else 0.0
        )
        lower_shadow = (
            round((min(latest_open, latest_close) - latest_low) / latest_close * 100, 2) if latest_close else 0.0
        )
        ...
        avg_dollar_volume20 = (
            round(cls._moving_average([closes[index] * volumes[index] for index in range(len(closes))], 20), 2)
    ...
    def _score_watchlist_quant_metrics(cls, *, metrics: dict[str, Any], strategy_profile: str) -> dict[str, Any]:
        ...
        if cls._safe_float(metrics.get("macdHist")) > 0:
            ...
            tags.append("MACD偏多")
        trend_score += (
            max(-5, min(6, (cls._safe_float(metrics.get("technicalScore")) - 55) / 8))
        ...
        if (
            cls._safe_float(metrics.get("lowerShadow")) >= cls._safe_float(metrics.get("upperShadow")) * 1.4
            and k_mid >= -1.0
        ):
        ...
        if 0 <= cci20 <= 180:
            ...
            momentum_score -= 4
        momentum_score += (
            max(-5, min(6, (cls._safe_float(metrics.get("momentumScore")) - 50) / 7))
            if metrics.get("momentumScore")
            else 0
        )
        ...
        if latest_close and ma20 and latest_close < ma20 and cls._safe_float(metrics.get("return20")) > -8:
            reversion_score += 4
        if (
            cls._safe_float(metrics.get("bollPercentB20"), 50.0) <= 18
            and cls._safe_float(metrics.get("dayChangePercent")) > 0
        ):
            reversion_score += 6
            tags.append("布林下轨反弹")
        ...
        trend_direction = (
            "up" if trend_score + momentum_score * 0.35 >= 22 else "down" if trend_score <= -5 else "sideways"
        )
    ...
    @classmethod
    def _safe_get_trend_scan(cls, symbol: str) -> dict[str, Any] | None:
        try:
    ...
    @staticmethod
    def _normalize_watchlist_symbol(raw_symbol: Any, market: str | None = None) -> str:
    ...
    def _build_alpha_factor_set(
        ...
        *,
        ...
        range_percent = [
            cls._safe_factor_ratio(highs[index] - lows[index], closes[index]) * 100 for index in range(count)
        ]
        body_percent = [
            cls._safe_factor_ratio(closes[index] - opens[index], opens[index]) * 100 for index in range(count)
        ]
        ...
        intraday_strength = [
            cls._safe_factor_ratio(
                (closes[index] - lows[index]) - (highs[index] - closes[index]), highs[index] - lows[index]
            )
            if highs[index] != lows[index]
            else 0.0
            for index in range(count)
        ...
        signed_volume = [
            volumes[index]
            if index > 0 and closes[index] > closes[index - 1]
            else -volumes[index]
            if index > 0 and closes[index] < closes[index - 1]
            else 0.0
            for index in range(count)
        ...
        def correlation(left: list[float], right: list[float], size: int, lag: int = 0) -> float:
            ...
            numerator = sum(
                (left_sample[index] - left_mean) * (right_sample[index] - right_mean) for index in range(sample_count)
            )
        ...
        for field_name, field_values in trend_fields.items():
            for size in windows:
                for lag in lag_range:
                    ...
                    add(
                        "trend",
                        f"{field_name}_ma_ratio_w{size}_l{lag}",
                        cls._safe_factor_ratio(current - average, average) * 100,
                    )
                    add(
                        "trend",
                        f"{field_name}_ma_slope_w{size}_l{lag}",
                        cls._safe_factor_ratio(average - previous_average, previous_average) * 100,
                    )
        for short_index, short_window in enumerate(windows[:-1]):
            for long_window in windows[short_index + 1 :]:
                ...
                add(
                    "trend",
                    f"close_ma_spread_w{short_window}_{long_window}",
                    cls._safe_factor_ratio(short_average - long_average, long_average) * 100,
                )
                add(
                    "trend",
                    f"close_ma_spread_rank_w{short_window}_{long_window}",
                    rank_percent(window(closes, long_window, 0), short_average),
                )
        ...
        for field_name, field_values in volatility_return_fields.items():
            for size in windows:
                for lag in lag_range:
                    ...
                    add(
                        "volatility",
                        f"{field_name}_ret_z_w{size}_l{lag}",
                        cls._safe_factor_ratio(current - average, dispersion),
                    )
        for size in windows:
            for lag in lag_range:
        ...
        for field_name, field_values in flow_fields.items():
            for size in windows:
                for lag in lag_range:
                    ...
                    add(
                        "volume_flow",
                        f"{field_name}_avg_ratio_w{size}_l{lag}",
                        cls._safe_factor_ratio(current - average, average) * 100,
                    )
                    add(
                        "volume_flow",
                        f"{field_name}_z_w{size}_l{lag}",
                        cls._safe_factor_ratio(current - average, dispersion),
                    )
                    add(
                        "volume_flow",
                        f"{field_name}_trend_w{size}_l{lag}",
                        cls._safe_factor_ratio(average - previous_average, previous_average) * 100,
                    )
            ...
            for size in windows:
                for lag in range(0, 2):
                    ...
                    add(
                        "price_action",
                        f"{field_name}_z_w{size}_l{lag}",
                        cls._safe_factor_ratio(current - mean(sample), dispersion),
                    )
        ...
        for field_name, field_values in range_fields.items():
            for size in long_windows:
                for lag in range(0, 3):
                    ...
                    add(
                        "range_drawdown",
                        f"{field_name}_pos_w{size}_l{lag}",
                        cls._safe_factor_ratio(current - low, high - low) * 100 if high != low else 50.0,
                    )
                    add(
                        "range_drawdown",
                        f"{field_name}_dist_high_w{size}_l{lag}",
                        cls._safe_factor_ratio(current - high, high) * 100,
                    )
                    add(
                        "range_drawdown",
                        f"{field_name}_dist_low_w{size}_l{lag}",
                        cls._safe_factor_ratio(current - low, low) * 100,
                    )
                    add(
                        "range_drawdown",
                        f"{field_name}_drawdown_w{size}_l{lag}",
                        cls._safe_factor_ratio(current - peak, peak) * 100,
                    )
        ...
        for field_name, field_values in liquidity_fields.items():
            for size in windows:
                for lag in lag_range:
                    ...
                    add(
                        "liquidity",
                        f"{field_name}_latest_to_avg_w{size}_l{lag}",
                        cls._safe_factor_ratio(current, average),
                    )
    ...
    @staticmethod
    def _vwap(
        highs: list[float], lows: list[float], closes: list[float], volumes: list[float], periods: int = 20
    ) -> float:
    ...
    def _money_flow_index(
        highs: list[float], lows: list[float], closes: list[float], volumes: list[float], periods: int = 14
    ) -> float:
        ...
        for index in range(start, len(closes)):
            typical = (float(highs[index] or 0.0) + float(lows[index] or 0.0) + float(closes[index] or 0.0)) / 3
            previous_typical = (
                float(highs[index - 1] or 0.0) + float(lows[index - 1] or 0.0) + float(closes[index - 1] or 0.0)
            ) / 3
    ...
    def _max_drawdown(values: list[float], periods: int = 20) -> float:
        sample = [
            float(item or 0.0) for item in (values[-periods:] if len(values) >= periods else values) if item is not None
        ]
        peak = 0.0
        max_drawdown = 0.0
        for value in sample:
    ...
    def _save_decision(
        ...
        return {**decision, "status": status, "orderId": order_id}

    @classmethod
    def _execute_decision(cls, account_id: int | None, decision: dict[str, Any], user_id: int = 1) -> dict[str, Any]:
        if not account_id:
    ...
    @classmethod
    def _load_broker_today_orders(cls, account_id: int | None, user_id: int) -> tuple[list[Any], str]:
        if not account_id:
            return [], "缺少账户"
        return cls._load_trade_service_orders(account_id=account_id, user_id=user_id)

    @classmethod
    def _find_active_broker_order(cls, orders: list[Any], symbol: str, side: str) -> dict[str, Any] | None:
    ...
    def _order_factor_inputs(cls, factor_inputs: Any) -> dict[str, Any]:
        ...
        if isinstance(factor_set, dict):
            ...
            compact["factorFamilies"] = (
                ...
                else factor_inputs.get("factorFamilies")
                if isinstance(factor_inputs.get("factorFamilies"), dict)
                else {}
            )
        return compact

    @classmethod
    def _submit_order_via_trade_service(
        cls, account_id: int, decision: dict[str, Any], user_id: int = 1
    ) -> dict[str, Any]:
        payload = {
        ...
        if response and response.get("success") is False:
            return {
                "status": "failed",
                "message": response.get("error") or response.get("message") or "trade-service 拒绝下单",
            }
    ...
    @staticmethod
    def _get_broker(account_id: int | None = None, user_id: int = 1):
        manager = get_broker_manager()
    ...
    def _compact_history_metrics(cls, metrics: Any) -> dict[str, Any]:
        ...
        if isinstance(factor_set, dict):
            ...
            compact["factorFamilies"] = (
                factor_set.get("families")
                if isinstance(factor_set.get("families"), dict)
                else compact.get("factorFamilies")
                if isinstance(compact.get("factorFamilies"), dict)
                else {}
            )
        return compact
