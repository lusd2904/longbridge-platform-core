...
class RecommendationService:
    PROFILE_MAP = {"growth": "成长型", "value": "价值型", "dividend": "稳健收益型", "momentum": "动量型"}
    ...
    @classmethod
    def get_latest(cls, profile: str = "growth", user_id: int = 1) -> dict | None:
        cls.ensure_schema()
    ...
    def _query_market_table(
        cls, market: str, table: str, name_field: str, asset_type: str, profile: str, limit: int
    ) -> list[dict]:
    ...
    def _score_candidate(cls, candidate: dict, profile: str) -> dict:
        ...
        candidate["risk_level"] = max(
            1, min(5, int(round(2 + abs(change) / 2.6 + (0 if candidate["asset_type"] == "etf" else 0.8))))
        )
...
"""
        ...
        confidence = (
            int(cls._extract_number(text, "置信度", candidate["confidence"])) if usable else candidate["confidence"]
        )
        ...
        candidate.update(
            {
                ...
                "risks": risks[:2],
                "ai_score": round((candidate["score"] * 0.55) + (float(ai_score) * 0.45), 2)
                if usable
                else round(candidate["score"], 2),
                "confidence": max(candidate["confidence"], min(98, confidence))
                if usable
                else int(candidate["confidence"]),
                "ai_generated": usable,
...
"""
    ...
    def _is_usable_ai_text(text: str) -> bool:
        ...
        if normalized.upper().startswith("ERROR"):
            return False
        matched_labels = sum(
            1 for marker in ["推荐摘要", "核心催化", "主要风险", "综合评分", "置信度"] if marker in normalized
        )
        if matched_labels < 3:
            return False
    ...
    @staticmethod
    def _minutes_since(timestamp: str | None) -> int:
        if not timestamp:
            return 999999
        try:
