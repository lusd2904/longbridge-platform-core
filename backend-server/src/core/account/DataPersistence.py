...
def _trim_text(value: Any | None, max_length: int | None = None) -> str:
    """清洗并截断文本，避免写库时超过字段容量。"""
...
class DataPersistenceManager:
    ...
    def get_latest_scan_result(self, symbol: str) -> ScanResult | None:
        """获取最新的扫描结果"""
        sql = """
        SELECT * FROM scan_results 
        WHERE symbol = %s
    ...
    def get_ai_analysis_history(
        ...
        limit: int = 100,
        offset: int = 0,
    ) -> tuple[list[AIAnalysisHistory], int]:
        """
        查询AI分析历史
    ...
    def get_latest_ai_analysis(self, symbol: str, user_id: int | None = None) -> AIAnalysisHistory | None:
        """获取最新的AI分析"""
        sql = """
        SELECT * FROM ai_analysis_history 
        WHERE symbol = %s
    ...
    def get_analysis_statistics(
        self, user_id: int | None = None, symbol: str | None = None, days: int = 30
    ) -> dict[str, Any]:
        """
        获取分析统计
