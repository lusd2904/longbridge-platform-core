...
from shared.longbridge import AdjustType, Period
    ...
    def scan_stocks(qc, us_pool, price_cache, holds):
        ...
        for s in us_pool:
            try:
                ...
                macd_tag = (
                    " <span style='color:#10b981;'>[↗️金叉]</span>"
                    if macd_h > 0
                    else " <span style='color:#ef4444;'>[↘️死叉]</span>"
                )
                ...
                hold_tag = " <b style='color:#3b82f6;'>[已持仓]</b>" if is_held else ""

                # 打印详细的股票信息
                MonitorLink.log(
                    f"📊 [扫描] {s} | 现价: {curr_p:.2f} | RSI: {rsi:.1f} | MACD: {macd_h:.3f} | KDJ: {k:.1f} | CCI: {cci:.1f} | 超买: {is_overbought} | 超卖: {is_oversold} | 持仓: {is_held}{hold_tag}{trend_tag}"
                )
                MonitorLink.log(
                    f"   [布林带] 中轨: {boll_mid:.2f} | 上轨: {boll_upper:.2f} | 下轨: {boll_lower:.2f}{bb_tag}"
                )
                MonitorLink.log(
                    f"   [趋势] EMA12: {ema_short:.2f} | EMA26: {ema_long:.2f}{ema_tag} | ROC: {roc:.2f}% | ATR: {atr:.2f}"
                )
                MonitorLink.log(f"   [支撑阻力] 支撑: {support:.2f} | 阻力: {resistance:.2f} | 市场环境: {market_env}")

                # 记录汇总数据
                summary_data.append(
                    f"{s}(P:{curr_p:.2f},RSI:{rsi:.1f},MACD:{macd_h:.2f},KDJ:{k:.1f},CCI:{cci:.1f},ATR:{atr:.2f},ROC:{roc:.2f},TREND:{trend})"
                )
