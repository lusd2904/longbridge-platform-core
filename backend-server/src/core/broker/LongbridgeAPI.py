...
from .BrokerInterface import AccountInfo, BaseBrokerAPI, Order, Position, Quote
...
class LongbridgeAPI(BaseBrokerAPI):
    ...
    def __init__(self, account_id: int):
        ...
        self.config: LongbridgeConfig | None = None
        self.trade_context: TradeContext | None = None
        self.quote_context: QuoteContext | None = None
    ...
    @classmethod
    def _record_error(cls, message: Any, account_id: int | None = None) -> None:
        with cls._metrics_lock:
    ...
    def _breaker(self, operation: str, failure_threshold: int = 4, recovery_timeout: int = 45) -> CircuitBreaker:
        return circuit_breakers.setdefault(
            ...
            CircuitBreaker(
                failure_threshold=failure_threshold, recovery_timeout=recovery_timeout, half_open_max_calls=2
            ),
        )
    ...
    def _load_cached_account_info(self) -> AccountInfo | None:
    ...
    def get_quote(self, symbols: list[str]) -> dict[str, Quote]:
        ...
        unique_symbols = list(
            dict.fromkeys([str(item or "").strip().upper() for item in symbols if str(item or "").strip()])
        )
        ...
        def fetch():
            ...
            if result:
                kafka_bus.publish_market_quotes(
                    account_id=self.account_id, quotes=[self._quote_to_cache_payload(item) for item in result.values()]
                )
    ...
    def get_account_info(self) -> AccountInfo:
        ...
        def fetch():
            ...
            redis_client.set_hot(
                f"broker:{self.account_id}:account", self._account_to_cache_payload(account_info), expire=20
            )
            return account_info
    ...
    def place_order(
        ...
        def submit():
            ...
            status = _enum_name(getattr(response, "status", None), default="SUBMITTED")
            result = {
                "order_id": order_id,
                "status": status,
                "symbol": symbol,
    ...
    @staticmethod
    def save_config(
        config: dict[str, str], user_id: int = 1, is_default: bool = False, account_row_id: int | None = None
    ) -> int:
        """
        保存配置到数据库
        ...
        try:
            if account_row_id:
                ...
                """
                params = (account_value, 1 if is_default else 0, account_row_id, user_id)
            ...
            params = (user_id, config.get("account", ""), 1 if is_default else 0)
