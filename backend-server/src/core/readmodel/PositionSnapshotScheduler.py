...
class PositionSnapshotScheduler:
    JOB_NAME = "position_snapshot_refresh"
        ...
        while not self._stop_event.is_set():
            try:
                ...
                if not SystemTaskService.is_enabled(self.JOB_NAME, default=True):
                    self._update_job("disabled", "持仓快照任务已关闭")
                ...
                for user_id in get_broker_manager().list_user_ids_with_accounts():
                    refreshed = PositionSnapshotService.refresh_for_user(user_id=user_id, source="scheduler")
                    account_count += len(refreshed)
                    position_count += sum(len(items) for items in refreshed.values())
                self._update_job("success", f"持仓快照已刷新，账户 {account_count}，持仓 {position_count}")
...
position_snapshot_scheduler = PositionSnapshotScheduler()
