...
class TradeExecutor:
    ...
    def execute_trade(engine, verdict, target, stock_data, holds, cash, max_positions=5):
        ...
        if verdict == "BUY":
            ...
            if qty > 0:
                MonitorLink.log(f"💰 [买入] {target} | 价格: ${curr_p:.2f} | 数量: {qty} | 成本: ${actual_buy_amount:.2f} | 手续费: ${commission_per_trade:.2f}")
            ...
            if qty > 0:
                ...
                MonitorLink.log(f"💰 [卖出] {target} | 价格: ${curr_p:.2f} | 数量: {qty} | 收入: ${sell_amount:.2f} | 手续费: ${commission_per_trade:.2f}")
                engine.execute_order(target, verdict, qty, curr_p)
                return True
