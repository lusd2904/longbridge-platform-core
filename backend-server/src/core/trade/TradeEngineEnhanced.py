...
from core.account.RiskManager import RiskConfig, RiskLevel, RiskManager
from shared.longbridge import OrderSide, OrderType, TimeInForceType
from utils.LoggerUtil import get_logger, log_error, log_risk_event, log_trade
...
class TradeEngineEnhanced:
    ...
    def __init__(self, tc, qc, risk_config: RiskConfig | None = None):
        """
        初始化增强版交易引擎
    ...
    def check_risk_before_order(
        self, side: str, symbol: str, quantity: int, price: float
    ...
    def buy(self, symbol: str, quantity: int, price: float, order_type: str = "LIMIT") -> dict[str, Any]:
        """
        买入股票（带风险控制和错误处理）
        ...
        try:
            ...
            order_result = self._submit_order(
                symbol=symbol, side=OrderSide.Buy, quantity=quantity, price=price, order_type=order_type
            )
            ...
            log_error(logger, e, {"context": "buy_order", "symbol": symbol, "quantity": quantity, "price": price})
            return {"success": False, "error": error_msg}

    def sell(self, symbol: str, quantity: int, price: float, order_type: str = "LIMIT") -> dict[str, Any]:
        """
        卖出股票（带风险控制和错误处理）
        ...
        try:
            ...
            if position["quantity"] < quantity:
                return {"success": False, "error": f'持仓不足: 持有{position["quantity"]}, 尝试卖出{quantity}'}
            ...
            order_result = self._submit_order(
                symbol=symbol, side=OrderSide.Sell, quantity=quantity, price=price, order_type=order_type
            )
            ...
            if order_result.get("success"):
                ...
                log_trade(
                    ...
                    price,
                    order_id=order_result.get("order_id"),
                    extra={"realized_pnl": realized_pnl, "entry_price": entry_price, "risk_level": risk_level.value},
                )
            ...
            log_error(logger, e, {"context": "sell_order", "symbol": symbol, "quantity": quantity, "price": price})
            return {"success": False, "error": error_msg}

    def _submit_order(
        self, symbol: str, side: OrderSide, quantity: int, price: float, order_type: str = "LIMIT"
    ) -> dict[str, Any]:
        """
        提交订单到交易所
    ...
    def get_account_info(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {"total_equity": total_equity, "total_cash": total_cash, "buying_power": total_cash}
