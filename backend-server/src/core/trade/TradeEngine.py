...
class TradeEngine:
    ...
    def sync_market_data(self, symbols):
        ...
        MonitorLink.log(f"🕐 [时间] 当前美东时间: {now.strftime('%H:%M:%S')} | 盘前:{is_pre_market} | 盘中:{is_regular_hours} | 盘后:{is_post_market} | 夜盘:{is_night_market}")
        ...
        try:
                    ...
                    try:
                        if self.qc:
                            ...
                            for q in quotes:
                                ...
                                # 添加调试信息，查看API返回的数据结构
                                MonitorLink.log(f"📊 [行情] {q.symbol} | last_done:{last_price} | prev_close:{prev_price} | pre_market:{pre_market_price} | post_market:{post_market_price} | final_price:{final_price}")
                        else:
                            # 使用tc获取行情数据
                            for symbol in batch:
                                try:
                                    ...
                                    if quote:
                                        ...
                                        # 添加调试信息，查看API返回的数据结构
                                        MonitorLink.log(f"📊 [行情] {symbol} | last_done:{last_price} | prev_close:{prev_price} | pre_market:{pre_market_price} | post_market:{post_market_price}")
            ...
            items = getattr(pos_resp, 'channels', [pos_resp])[0].positions if hasattr(pos_resp, 'channels') else pos_resp
            ...
            # 打印账户信息
            MonitorLink.log(f"💰 [账户] 现金: HK${cash_total_hkd:.2f} (${cash_total_usd:.2f} USD) | 总资产: ${total_assets:.2f} USD")
            ...
            for o in orders:
                ...
                if o.status in [1, 2, 3, 4]:
                    ...
                    if submitted_at and (current_time - submitted_at) > AppConfig.get("CANCEL_ORDER_THRESHOLD_SECONDS"):
                        self.tc.cancel_order(o.order_id)
                        cancel_count += 1
                        MonitorLink.log(f"🗑️ [撤单策略] 撤销订单: {o.order_id} ({o.symbol})，已超时 {int(current_time - submitted_at)} 秒。")
            if cancel_count > 0:
    ...
    def get_stock_quote(self, symbol):
        ...
        try:
            ...
            if self.qc:
                MonitorLink.log(f"✅ [获取行情] QuoteContext 可用")
                try:
                    ...
                    if quotes:
                        ...
                        MonitorLink.log(f"📊 [实时行情] {symbol} | 价格:{final_price:.2f} | 涨跌幅:{change_percent:+.2f}%")
                        return stock_data
                    else:
