...
class CopyTradingService:
    ...
    def update_trader_stats(self, user_id: str, return_rate: float, win_rate: float, drawdown: float):
        """更新交易员统计"""
    ...
    def _calculate_tier(self, trader: TraderProfile) -> TraderTier:
        """计算交易员等级"""
        # 根据收益、胜率、回撤综合评分
        score = trader.total_return * 0.4 + trader.win_rate * 100 * 0.3 + (1 - trader.max_drawdown) * 100 * 0.3
    ...
    def _generate_copy_order(self, signal: TradeSignal, config: CopyTradeConfig) -> dict | None:
        """生成跟单订单"""
        # 计算跟单数量
    ...
    def get_trader_ranking(self, sort_by: str = "total_return", limit: int = 100) -> list[TraderProfile]:
        """
        获取交易员排行榜
    ...
    def get_trader_detail(self, trader_id: str) -> dict | None:
        """获取交易员详情"""
        if trader_id not in self.traders:
            return None
        ...
        # 获取最近交易信号
        recent_signals = [signal for signal in self.trade_signals if signal.trader_id == trader_id][-20:]
    ...
    def get_portfolio_shares(self, sort_by: str = "likes", limit: int = 50) -> list[PortfolioShare]:
        """获取组合分享列表"""
    ...
    def get_copy_performance(self, follower_id: str, trader_id: str) -> dict:
        """获取跟单绩效"""
        config_id = f"{follower_id}_{trader_id}"
        ...
        # 统计跟单信号
        copied_signals = [
            signal for signal in self.trade_signals if signal.trader_id == trader_id and follower_id in signal.copied_by
        ]
