...
from core.broker.BrokerInterface import get_broker_manager, BrokerType
...
def _get_user_broker_account(account_id: int, user_id: int, broker_type: Optional[str] = None) -> Optional[Dict[str, Any]]:
    sql = """
    SELECT *
...
def _get_quote_last_price(quote: Any) -> Optional[float]:
...
def _load_reference_price(broker, symbol: str, price: Optional[float]) -> Tuple[Optional[float], Dict[str, Any]]:
...
def _run_order_risk_check(broker, symbol: str, action: str, quantity: int, reference_price: float) -> Tuple[bool, str, RiskLevel]:
...
def list_broker_providers():
    ...
    try:
        ...
        return jsonify({
            'success': True,
            'data': manager.list_supported_brokers()
        })
    except Exception as e:
...
def get_default_account():
    ...
    try:
        ...
        return jsonify({
            'success': True,
            'data': _mask_and_enrich_account(default_account)
        })
    except Exception as e:
...
def save_longbridge_config():
    ...
    try:
        ...
        config = {
            'account': str(data.get('account', '')).strip() or cli_account_no()
        }
...
def test_connection(account_id: int):
    ...
    try:
        ...
        if broker_type == 'longbridge':
            ...
            return jsonify({
                'success': False,
                'message': f'不支持的券商类型: {broker_type}'
            }), 400
        
        # 检查配置是否加载
        if hasattr(broker, 'config'):
            logger.info(f"broker配置已加载")
        else:
...
@login_required
@rate_limit(key_func=lambda: f"trade-submit:{getattr(request, 'user_id', 'anonymous')}:{request.view_args.get('account_id', 'default')}", limit=10, window=60)
    ...
    try:
        ...
        try:
            _audit_trade_event(
                ...
                request_price=float((request.get_json(silent=True) or {}).get('price')) if (request.get_json(silent=True) or {}).get('price') not in (None, '') else None,
...
@login_required
@rate_limit(key_func=lambda: f"trade-cancel:{getattr(request, 'user_id', 'anonymous')}:{request.view_args.get('account_id', 'default')}", limit=10, window=60)
...
def get_quotes():
    ...
    try:
        ...
        # 转换为字典
        quotes_data = {
            symbol: _serialize_quote(symbol, q)
