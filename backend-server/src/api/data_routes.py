...
from core.broker.BrokerInterface import get_broker, get_broker_manager
...
def _resolve_account_id(account_id: int | None, user_id: int) -> int | None:
    if account_id:
        return int(account_id)
...
def _get_connected_broker(account_id: int | None = None, user_id: int | None = None, require_connection: bool = True):
    """获取并连接券商实例。"""
...
def _build_account_summary(account_info, positions) -> dict[str, Any]:
    def read_value(position, attr_name: str, fallback_name: str | None = None) -> float:
...
def _build_snapshot_summary(account_id: int | None, user_id: int, positions: list[dict[str, Any]]) -> dict[str, Any]:
...
def _load_realtime_account_state(
    account_id: int | None = None, user_id: int | None = None
    ...
    return {
        **position,
        ...
        "available_quantity": float(
            position.get("availableQuantity") or position.get("available_quantity") or quantity
        ),
...
def _table_exists(table_name: str) -> bool:
    row = DbUtil.query_one(
        "SELECT 1 FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name = %s", (table_name,)
    )
    return bool(row)
...
def _fetch_stock_pool_rows(
    table_config: dict[str, str], user_id: int, search: str = "", group_id: str = ""
) -> list[dict[str, Any]]:
...
def _count_stock_pool_rows(table_config: dict[str, str], user_id: int, group_id: str = "") -> int:
    table_name = table_config["table"]
    if not _table_exists(table_name):
        return 0
    ...
    row = DbUtil.query_one(f"SELECT COUNT(1) FROM {table_name} {where_clause}", tuple(params))
    return int(row[0] or 0) if row else 0


def _build_stock_pool_stats(user_id: int, group_id: str = "") -> dict[str, Any]:
    stats = {"total": 0, "stocks": 0, "etfs": 0, "markets": {"US": 0, "CN": 0, "HK": 0}}
...
def _merge_quote_snapshot(base: dict[str, Any], snapshot: dict[str, Any] | None = None) -> dict[str, Any]:
    merged = dict(base or {})
...
def _candidate_quote_accounts(user_id: int, account_id: int | None = None) -> list[dict[str, Any]]:
    ...
    for account in manager.list_accounts(user_id=user_id) or []:
        candidate = {"id": int(account.get("id") or 0), "user_id": user_id, "broker_type": account.get("broker_type")}
        if candidate["id"]:
...
def _fetch_live_quotes(symbols: list[str], user_id: int, account_id: int | None = None) -> dict[str, dict[str, Any]]:
    normalized_symbols = []
...
def _ensure_risk_control_tables() -> None:
    ...
    try:
        ...
        if not column:
            DbUtil.execute_sql(
                "ALTER TABLE user_risk_orders ADD COLUMN strategy_id INT NOT NULL DEFAULT 0 AFTER quantity"
            )
        else:
        ...
        strategy_unique = DbUtil.fetch_one(
            "SHOW INDEX FROM user_risk_orders WHERE Key_name = 'uniq_user_strategy_symbol_type'"
        )
        ...
        index_row = DbUtil.fetch_one(
            "SHOW INDEX FROM user_risk_orders WHERE Key_name = 'idx_user_strategy_type_status'"
        )
        if not index_row:
            DbUtil.execute_sql(
...
def _load_position_snapshot(user_id: int, account_id: int | None = None) -> dict[str, dict[str, Any]]:
...
def _load_risk_orders(user_id: int, order_type: str, account_id: int | None = None) -> list[dict[str, Any]]:
    _ensure_risk_control_tables()
    rows = (
        DbUtil.fetch_all(
            """
    ...
    for row in rows:
        ...
        current_price = float(
            ...
            or _lookup_universe_snapshot(symbol).get("price")
            or 0
        )
...
def _build_risk_overview(user_id: int, account_id: int | None = None) -> dict[str, Any]:
...
def _get_notification_states(user_id: int, keys: list[str]) -> dict[str, dict[str, Any]]:
    ...
    return {
        row.get("notification_key"): {"is_read": bool(row.get("is_read")), "is_hidden": bool(row.get("is_hidden"))}
        for row in rows
    }


def _upsert_notification_states(
    user_id: int, keys: list[str], is_read: bool | None = None, is_hidden: bool | None = None
) -> int:
    _ensure_notification_state_table()
    affected = 0
    for key in keys:
...
def _collect_notifications(user_id: int, limit: int = 50, notification_type: str = "") -> list[dict[str, Any]]:
    ...
    if notification_type in {"", "risk"}:
        ...
        if notification_type == "risk":
            items.extend(
                _collect_agent_review_notifications(
                    user_id=user_id,
                    limit=max(8, limit // 2),
                    risk_only=True,
                )
...
def _collect_agent_review_notifications(user_id: int, limit: int = 20, risk_only: bool = False) -> list[dict[str, Any]]:
    try:
        rows = AgentRunService.list_recent_runs(
            user_id=user_id, limit=max(1, min(int(limit or 20), 40)), use_primary=True
        )
    except Exception as exc:
    ...
    try:
        ...
        for table_config in _iter_stock_pool_tables(market):
            try:
                ...
                stocks.extend(
                    {"symbol": row["symbol"], "name": row["name"], "market": row["market"], "type": row["type"]}
                    for row in rows[:50]
                )
            except Exception as exc:
                print(f"❌ [API] 从 {table_config['table']} 获取失败: {exc}")
        ...
        return jsonify({"success": True, "stocks": unique_stocks})
    except Exception as e:
...
def get_stock_quote(symbol):
    ...
    try:
        ...
        # 获取账户信息
        db = DbUtil()
        row = db.fetch_one("SELECT broker_type FROM broker_accounts WHERE id = %s AND is_active = 1", (account_id,))
        if not row:
...
def get_stock_quotes():
    ...
    try:
        ...
        live_quotes = _fetch_live_quotes(
            symbols, user_id=request.user_id, account_id=int(broker_account_id) if broker_account_id else None
        )
        quotes = []
        for symbol in symbols:
            normalized_symbol = _normalize_market_symbol(symbol)
            ...
            if change is None:
                change = (
                    ...
                    if prev_close
                    else (last_price * float(change_percent or 0) / 100 if last_price else 0)
                )
            quotes.append(
                {
                    "symbol": normalized_symbol,
...
def get_market_status():
    ...
    try:
        ...
        def get_market_info(timezone_str, open_hour, open_minute, close_hour, close_minute, market_name):
            ...
            return {"status": status, "status_text": status_text, "current_time": current_time}
        ...
        cn_info = {"status": cn_status, "status_text": cn_status_text, "current_time": cn_time}
        ...
        hk_info = {"status": hk_status, "status_text": hk_status_text, "current_time": hk_time}

        return jsonify({"success": True, "data": {"US": us_info, "CN": cn_info, "HK": hk_info}})
    except Exception as e:
...
def refresh_dashboard_market_insights():
    ...
    try:
        ...
        return jsonify({"success": True, "data": result.get("markets", []), "generated_at": result.get("generated_at")})
    except Exception as e:
...
def get_data():
    ...
    try:
        ...
        return jsonify({"scans": scans, "ais": ais, "account": account, "holdings": holdings, "orders": orders})
    except Exception as e:
        print(f"获取数据失败: {e}")
...
def get_dashboard_summary():
    ...
    try:
        ...
        if not _should_force_realtime():
            ...
            snapshot = (
                AccountAssetSnapshotService.get_latest(
                    user_id=request.user_id,
...
def get_market_history():
    ...
    try:
        ...
        payload = HistoricalMarketDataService.get_history(
            symbol=symbol, timeframe=timeframe, limit=limit, user_id=request.user_id, refresh=refresh
        )
...
def get_market_history_compare():
    ...
    try:
        ...
        payload = HistoricalMarketDataService.get_compare_history(
            symbols=raw_symbols, timeframe=timeframe, limit=limit, user_id=request.user_id, refresh=refresh
        )
        return jsonify({"success": True, "data": payload})
    except ValueError as exc:
        return jsonify({"success": False, "error": str(exc)}), 400
...
def get_accounts():
    ...
    try:
        ...
        for account in accounts:
            ...
            masked_account_id = (
                account_id[:2] + "*" * (len(account_id) - 4) + account_id[-2:]
...
def run_strategy_monitor():
    ...
    try:
        ...
        result = StrategyMonitorService.run_monitor(
            user_id=request.user_id, account_id=int(account_id) if account_id else None, source="manual"
        )
        return jsonify({"success": True, "data": result})
    except ValueError as e:
        return jsonify({"success": False, "error": str(e)}), 400
...
def read_notification():
    try:
        data = request.get_json(silent=True) or {}
        keys = (
            data.get("keys") if isinstance(data.get("keys"), list) else [data.get("notification_key") or data.get("id")]
        )
