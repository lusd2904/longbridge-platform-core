...
def run_platform_task(task_key):
    try:
        if task_key == "finance_briefing_refresh":
            ...
            result = MarketHistoryBootstrapService.run_once(
                user_id=request.user_id, batch_size=SystemTaskService.get_batch_size(task_key, 160)
            )
        else:
...
def get_symbol_overview(symbol):
    try:
        ...
        history = HistoricalMarketDataService.get_history(
            normalized_symbol, timeframe="daily", limit=120, user_id=request.user_id
        )
        ...
        market_insights = {
            item["market"]: item for item in MarketInsightService.get_latest_snapshots(user_id=request.user_id)
        }
        ...
        system_items = [
            {
                ...
                "level": "error"
                if "失败" in (row.get("log_content") or "") or "ERROR" in (row.get("log_content") or "")
                else "info",
                "module": "system",
                "operation": "runtime",
