...
from utils.cache import AICache, StockCache, cache
...
def _get_quote_from_broker(symbol, account_id=None, user_id=None):
    ...
    MonitorLink.log(f"✅ 获取实时行情: {quote['symbol']} = {quote['last_price']} ({quote['change_percent']:+.2f}%)")
    return (quote["last_price"], quote["volume"], quote["change_percent"], quote["prev_close"])
...
def _build_real_indicator_context(symbol, current_price, volume, user_id=1):
    ...
    indicator_payload = {
        ...
        "ema": {"short": float(daily_snapshot.get("emaShort") or 0), "long": float(daily_snapshot.get("emaLong") or 0)},
...
def _build_trend_scan_analysis_result(scan):
    ...
    signal_map = {"up": "success", "down": "danger", "sideways": "warning"}
    decision_map = {"up": "偏多", "down": "偏空", "sideways": "观望"}
...
def get_latest_trend_scans():
    try:
        ...
        items = DailySymbolTrendScanService.get_latest_batch(
            symbols=raw_symbols or None, market=market or None, limit=limit
        )
        return jsonify({"success": True, "data": [_build_trend_scan_analysis_result(item) for item in items]})
    except Exception as exc:
        return jsonify({"success": False, "error": str(exc)}), 500
...
def refresh_recommendations():
    try:
        ...
        user_id = getattr(request, "user_id", 1)
        result = RecommendationService.refresh(profile=profile, user_id=user_id, force=True)
        return jsonify({"success": True, "message": "智能推荐已刷新", "data": result})
    except Exception as exc:
        return jsonify({"success": False, "error": str(exc)}), 500
...
def run_quant_cycle():
    try:
        ...
        return jsonify({"success": True, "message": "自选池量化策略扫描已完成", "data": result})
        ...
        for position in positions:
            ...
            try:
                ...
                if cached_result and cached_result.get("scanLayers"):
                    current_price, volume, change_percent, prev_close = _get_quote_from_broker(
                        symbol, account_id, user_id=user_id
                    )
                ...
                if current_price <= 0:
                    fallback_price, fallback_volume, fallback_change, fallback_prev_close = (
                        _extract_position_quote_fallback(position)
                    )
                    if fallback_price > 0:
                ...
                # 5. 构建分析结果
                signal_map = {"BUY": "success", "SELL": "danger", "HOLD": "warning"}
                decision_map = {"BUY": "买入", "SELL": "卖出", "HOLD": "观望"}
    ...
    try:
        ...
        if cached_indicators:
            ...
            if len(valid_items) < 20:
                raise ValueError(
                    f"{HistoricalMarketDataService.normalize_symbol(symbol)} 历史行情不足，无法计算技术指标"
                )
            ...
            indicators = {
                ...
                "ema": {"short": ema_short, "long": ema_long},
                "atr": atr,
                "roc": roc,
                "cci": cci,
                "obv": obv,
...
def batch_analyze_positions():
    ...
    try:
        ...
        # 第一层：批量获取行情数据（串行）
        MonitorLink.log("   [第1层] 开始批量获取行情数据")
        stock_data_list = []
        error_symbols = []
        ...
        MonitorLink.log(
            f"   [第1层] 完成，成功获取 {len(stock_data_list)} 只股票的行情，{len(error_symbols)} 只股票无行情"
        )
        ...
        # 第二层：批量计算技术指标（串行）
        if stock_data_list:
            MonitorLink.log("   [第2层] 开始批量计算技术指标")
            indicators_list = []
            ...
            if indicators_list:
                # 第三层：批量AI模型分析（串行）
                MonitorLink.log("   [第3层] 开始批量AI模型分析")
                ai_analysis_list = []
                ...
                if ai_analysis_list:
                    # 第四层：批量生成综合决策（串行）
                    MonitorLink.log("   [第4层] 开始批量生成综合决策")
