...
def _proxy_trade_service(path, method="GET", payload=None, params=None):
    ...
    try:
        ...
        return jsonify({"success": False, "error": f"trade-service 不可用: {exc}"}), 502

    try:
        body = response.json()
    except ValueError:
        body = {"success": response.ok, "error": response.text or "trade-service 返回了非 JSON 响应"}
...
def get_account_config(account_id, user_id=None):
    ...
    try:
        ...
        broker_name = {"longbridge": "长桥", "tiger": "老虎", "interactive_brokers": "盈透"}.get(
...
def get_real_positions():
    ...
    try:
                ...
                positions.append(
                    {
                        ...
                        "pnl_ratio": (p.unrealized_pnl / (p.average_cost * p.quantity) * 100)
                        if p.average_cost > 0 and p.quantity > 0
                        else 0,
                        "currency": "USD",
                        "account_id": account_id,
                        "account_name": config.get("account_name", ""),
        ...
        return jsonify({"success": True, "positions": positions, "count": len(positions), "account_id": account_id})
...
def get_all_positions(user_id):
    ...
    try:
                        ...
                        for channel in pos_resp.channels:
                            if hasattr(channel, "positions"):
                                for p in channel.positions:
                                    ...
                                    pnl = (current_price - cost_price) * qty if current_price > 0 else 0
                                    pnl_ratio = (
                                        ((current_price - cost_price) / cost_price * 100) if cost_price > 0 else 0
                                    )
        ...
        return jsonify({"success": True, "positions": all_positions, "count": len(all_positions)})
...
def get_real_orders():
    ...
    try:
        ...
        return jsonify({"success": True, "orders": orders, "count": len(orders), "account_id": account_id})
...
def buy_stock():
    ...
    try:
        ...
        if broker_type == "longbridge":
            ...
            # 老虎证券交易
            resp = tc.place_order(
                symbol=symbol, action="BUY", quantity=quantity, order_type="LIMIT", price=price, time_in_force="DAY"
            )
...
def sell_stock():
    ...
    try:
        ...
        if broker_type == "longbridge":
            ...
            # 老虎证券交易
            resp = tc.place_order(
                symbol=symbol, action="SELL", quantity=quantity, order_type="LIMIT", price=price, time_in_force="DAY"
            )
...
def cancel_order():
    ...
    if settings.TRADE_SERVICE_ENABLED:
        ...
        return _proxy_trade_service(
            "/orders/cancel",
            method="POST",
            payload={"order_id": data.get("order_id"), "account_id": data.get("account_id")},
        )
    ...
    try:
        ...
        if broker_type == "longbridge":
            ...
            return jsonify(
                {"success": True, "message": f"订单 {order_id} 已撤销", "order_id": order_id, "account_id": account_id}
            )
        elif broker_type == "tiger":
            # 老虎证券撤单
        ...
        if broker_type == "longbridge":
            ...
            try:
                ...
                if quotes and len(quotes) > 0:
                    ...
                    return jsonify(
                        {"success": False, "error": f"无法获取股票 {symbol} 的行情，请检查股票代码是否正确"}
                    ), 400
            except Exception as e:
            ...
            try:
                    ...
                    return jsonify(
                        {
                            ...
                            "data": {
                                ...
                                "change_percent": (
                                    (q.get("last_price", 0) - q.get("prev_close", 0)) / q.get("prev_close", 0) * 100
                                )
                ...
                if "permission denied" in error_msg.lower() or "do not have permissions" in error_msg.lower():
                    return jsonify(
                        {"success": False, "error": "老虎证券账户没有美股行情权限，请在老虎证券官网开通行情权限后再试"}
                    ), 403
